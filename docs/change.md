...
- MQTT 连接成功后仍立即发送一次 `device_status`。
- 后续心跳从固定 60 秒改为每轮独立生成的 `50-70 秒`随机周期，降低批量上电、断网恢复和 OTA 重启后的同步洪峰。
- 随机数使用 ESP-IDF `esp_random()`；不改变 MQTT KeepAlive、心跳消息格式和云端 150 秒异常阈值。
- 影响文件：`main/application.cc`、`main/application.h`。
